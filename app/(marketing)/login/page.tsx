...
export default function LoginPage() {
  const [state, action, pending] = useActionState(login, { error: null as string | null })
    ...
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-b from-brand-dark to-[#2d0030] px-4">
      <Card className="w-full max-w-sm border-0 bg-white/95 p-0 shadow-2xl backdrop-blur-sm">
        <CardHeader className="items-center pt-10 pb-6 text-center">
          ...
          <CardTitle className="text-2xl font-semibold text-brand-dark">Welcome back</CardTitle>
          <p className="mt-1 text-sm text-brand-dark/50">Sign in to continue learning</p>
        </CardHeader>
        <CardContent className="px-8 pb-10">
          ...
          <p className="mt-6 text-center text-sm text-brand-dark/50">
            ...
            <Link href="/signup" className="font-medium text-brand-dark underline-offset-2 hover:text-brand-gold-ink hover:underline">
              Sign up
            </Link>
          </p>
        </CardContent>
